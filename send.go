...
const UrlQueryUser = UrlBase + "/api/fun/wxuser/?appToken=${appToken}"
...
func QueryMessageStatus(messageId int) (*QueryMsgResult, error) {
...
func QueryAllUser(appToken string) (*QueryUsersResult, error) {
	var result QueryUsersResult
	url := strings.ReplaceAll(UrlQueryUser, "${appToken}", appToken)
	resp, err := http.Get(url)
	if err != nil {
		return &result, NewBusinessError(err)
	}
